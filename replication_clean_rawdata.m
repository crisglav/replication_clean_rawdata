...
% Loop over channelcriterions
% channelcriterion = [0.7 0.8 0.9];
channelcriterion = 0.8; % default
for iC=1:length(channelcriterion)
    ...
    for iRep =1:nReps
        ...
        EEG = pop_clean_rawdata(EEG,'FlatlineCriterion',5,'ChannelCriterion',channelcriterion(iC),...
    ...
    sgtitle(['ChannelCriterion = ' num2str(channelcriterion(iC))]);
%     saveas(f,['replication_CRD_' num2str(channelcriterion(iC)) '.png']);
    saveas(f,['replication_CRD_' num2str(channelcriterion(iC)) '_nSamples' num2str(numsamples) '.png']);
